...
public class MailSender {
	...
	public String mailSend(String email) {
		...
		try {
			...
			// 보내는사람 정보
			msg.setFrom(new InternetAddress("connectshop91@gmail.com", "집쿡"));
			// 받는사람정보
			InternetAddress to = new InternetAddress(email);
			msg.setRecipient(Message.RecipientType.TO, to);
			// 이메일 제목설정
			msg.setSubject("[집쿡] 이메일 인증을 위한 인증번호를 안내드립니다.", "UTF-8");
			// 이메일 내용설정
			msg.setContent("<h1>안녕하세요. 집쿡입니다.</h1>"+"<h3>인증번호는["+sb.toString()+"] 입니다.</h3>", "text/html;charset=UTF-8");
			// 이메일 전송
			Transport.send(msg);
			result = true;
